...
class User extends Authenticatable implements JWTSubject
{
	...
	public function sendPasswordResetNotification($token)
	{
		if ($this->group_user_id == 1) {
			$this->notify(new \App\Notifications\Admin\PasswordReset(route('admin.show_reset_form', $token)));

		} else {

			$this->notify(new \App\Notifications\Admin\PasswordReset($this->store->domainUrl().'/password/reset/'.$token));
