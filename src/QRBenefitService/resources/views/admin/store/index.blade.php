			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<a class="btn btn-info" href="{{ route('admin_store_show_view_add') }}" data-shuffle=""> Add </a>
						<button type="button" data-toggle="tooltip" title="Delete" class="btn btn-danger" onclick="confirm('Are you sure') ? $('#form-store').submit() : false;"> Delete</button>
					</div>
					<div class="card-header">
						<form action="{{ route('admin_store_index') }}" method="GET" id="form-filter">
					...
					<!-- /.card-header -->
					<div class="card-body">
						<form action="{{ route('admin_store_delete',array_merge($filter_parameters, $order_parameters)) }}" method="post" enctype="multipart/form-data" id="form-store">
							...
							@csrf
												...
												<thead>
													<tr role="row">
														...
														@if ( isset($order_parameters['sort']) && $order_parameters['sort'] == 'id' && $order_parameters['order'] == 'asc')
															<a href="{{ route('admin_store_index',array_merge($filter_parameters,['sort' => 'id', 'order' => 'desc'])) }}" data-shuffle=""> Id </a>
														@else
															<a href="{{ route('admin_store_index',array_merge($filter_parameters,['sort' => 'id', 'order' => 'asc'])) }}" data-shuffle=""> Id </a>
														@endif
														</th>
														<th class="sorting @if ( isset($order_parameters['sort']) && $order_parameters['sort'] == 'name') sorting_{{$order_parameters['order']}}  @endif" tabindex="0" aria-controls="example2" rowspan="1"
															...
															@if ( isset($order_parameters['sort']) && $order_parameters['sort'] == 'name' && $order_parameters['order'] == 'asc')
																<a href="{{ route('admin_store_index',array_merge($filter_parameters,['sort' => 'name', 'order' => 'desc'])) }}" data-shuffle=""> Full Name </a>
															@else
																<a href="{{ route('admin_store_index',array_merge($filter_parameters,['sort' => 'name', 'order' => 'asc'])) }}" data-shuffle=""> Full Name </a>
															@endif
														</th>
														<th class="sorting @if ( isset($order_parameters['sort']) && $order_parameters['sort'] == 'email') sorting_{{$order_parameters['order']}}  @endif" tabindex="0" aria-controls="example2" rowspan="1"
															...
															@if ( isset($order_parameters['sort']) && $order_parameters['sort'] == 'email' && $order_parameters['order'] == 'asc')
																<a href="{{ route('admin_store_index',array_merge($filter_parameters,['sort' => 'email', 'order' => 'desc'])) }}" data-shuffle=""> Email </a>
															@else
																<a href="{{ route('admin_store_index',array_merge($filter_parameters,['sort' => 'email', 'order' => 'asc'])) }}" data-shuffle=""> Email </a>
															@endif
														</th>
														...
														colspan="1">
															@if ( isset($order_parameters['sort']) && $order_parameters['sort'] == 'phone_number' && $order_parameters['order'] == 'asc')
																<a href="{{ route('admin_store_index',array_merge($filter_parameters,['sort' => 'phone_number', 'order' => 'desc'])) }}" data-shuffle=""> Phone Number </a>
															@else
																<a href="{{ route('admin_store_index',array_merge($filter_parameters,['sort' => 'phone_number', 'order' => 'asc'])) }}" data-shuffle=""> Phone Number  </a>
															@endif
														</th>
												...
												<tbody>
													@foreach ($stores as $store)
														<tr class="odd">
															...
															<td>
															@if (!empty($store->logo))
															<img src="{{ asset('storage/'.$store->logo) }}" style="width: 60px;">
															@endif
															</td>
															<td>{{ $store->default_tax }}</td>
															<td class="text-center">
																<a class="btn btn-info m-1" href="{{ route('admin_store_show_view_info', ['id' =>$store->store_id ]) }}" data-shuffle="">
																	<i class="fa fa-eye" aria-hidden="true"></i>
																</a>
																<a class="btn btn-info m-1" href="{{ route('admin_store_show_view_edit', ['id' =>$store->store_id ]) }}" data-shuffle="">
								...
								<div class="row">
									<div class="col-sm-12 col-md-5">
										...
										@if ($stores->count() > 0)
											Showing {{ $stores->currentPage()  }} to {{ (($stores->currentPage() - 1) * $stores->perPage()) + $stores->count() }} of {{ $stores->total() }} entries
										@endif
										</div>
									</div>
									<div class="col-sm-12 col-md-7">
										<div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">
										{!! $stores->render('admin.pagination.bootstrap-4') !!}
										</div>
									</div>
								</div>
							</div>
