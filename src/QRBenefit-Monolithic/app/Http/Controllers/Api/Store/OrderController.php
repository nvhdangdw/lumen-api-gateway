...
class OrderController extends Controller
{
	...
	public function index(Request $request) {
		...
		foreach ($orders as $order) {
			...
			$row = array(
				...
				"vouchers_redeemed" => $order->vouchers_redeemed,
	...
	public function create(Request $request) {
		...
		if( !empty($order_data['discount_ids']) ) {
			$vouchers_redeemed = count($order_data['discount_ids']);
		...
		$order->vouchers_redeemed = $vouchers_redeemed;
	...
	// 	$transaction_fields = ["id","total_tax","total_discount","total_amount","date","customer_name","store_name","vouchers_redeemed"];
	...
	// 	if (isset($request->sort) && !empty($request->sort) && in_array($request->sort, $transaction_fields)) {
	// 		$transaction = "asc";
	// 		if (isset($request->transaction) && !empty($request->transaction) && strtolower($request->transaction) == "desc") {
	// 			$transaction = "desc";
	// 		}
	// 		$transaction_parameters['sort'] = $request->sort;
	// 		$transaction_parameters['transaction'] = $transaction;
	// 		if ($request->sort == 'id'){
	// 			$query->orderBy("transaction.transaction_id",$transaction);
	// 		} else if ($request->sort == 'date'){
	// 			$query->orderBy("transaction.created_at",$transaction);
	// 		} else if ($request->sort == 'customer_name'){
	// 			$query->orderBy("customer_name",$transaction);
	// 		} else if ($request->sort == 'store_name') {
	// 			$query->orderBy("store.name",$transaction);
	// 		} else {
	// 			$query->orderBy("transaction.".$request->sort,$transaction);
	// 		}
	// 	}
	// 	if (empty($transaction_parameters))
	// 		$query->orderBy('transaction.created_at','decs');
	// 	$transactions = $query->paginate($this->perPage);
	// 	// add filter to link pagination
	// 	$transactions->appends(array_merge($filter_parameters, $transaction_parameters))->links();
	// 	$data = array();
	...
	// 		$data[] = $row;
	// 	}
	// 	$transactions->setCollection(collect($data));
