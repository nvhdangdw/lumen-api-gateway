...
class RewardIncrementLarge extends Mailable
{
    ...
    public function build()
    {
        if (file_exists( resource_path() . '/views/email/store/'. Auth::user()->store->code()) && !empty(Auth::user()->store->code())) {
			return $this->view('email.store.'. Auth::user()->store->code() .'.transaction.reward_increment_large');
        } else {
            return $this->view('email.store.common.transaction.reward_increment_large');
        }
    }
}