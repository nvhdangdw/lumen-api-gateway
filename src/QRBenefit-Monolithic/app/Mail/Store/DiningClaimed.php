...
class DiningClaimed extends Mailable
{
    ...
    public function build()
    {
        if (file_exists( resource_path() . '/views/email/store/'. Auth::user()->store->code()) && !empty(Auth::user()->store->code())) {
            return $this->view('email.store.'. Auth::user()->store->code() .'.transaction.dining_claimed');
        } else {
            return $this->view('email.store.common.transaction.dining_claimed');
        }
    }
}